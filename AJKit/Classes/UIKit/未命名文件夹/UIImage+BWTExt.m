...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _BWT_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)_BWT_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
- (UIImage *)_BWT_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
...
- (UIImage *)imageByRotate180 {
    return [self _BWT_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self _BWT_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self _BWT_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _BWT_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_BWT_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _BWT_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
static void _BWT_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}

// Helper function to add tint and mask.
- (UIImage *)_BWT_mergeImageRef:(CGImageRef)effectCGImage
                      tintColor:(UIColor *)tintColor
...
- (UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGSize)newSize
{
    ...
    UIGraphicsBeginImageContextWithOptions(newSize, NO, [UIScreen mainScreen].scale * 2);
...
+ (UIImage *)imageWithBase64StringQRCode:(NSString *)stringQRCode size:(CGFloat)size {
    stringQRCode = [stringQRCode base64DecodedToHex];
    return [self imageWithStringQRCode:stringQRCode size:size];
}

+ (UIImage *)imageWithStringQRCode:(NSString *)string size:(CGFloat)size {
    // 生成二维码，模糊的
    CIFilter *filter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    [filter setDefaults];
    NSData *data = [self p_stringToHex:string];
