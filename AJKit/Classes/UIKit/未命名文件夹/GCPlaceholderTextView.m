...
- (void)drawRect:(CGRect)rect{
    if(self.placeholder_Draw && self.text.length == 0 && _isNeedDraw) {
        NSString *str = self.placeholder_Draw;

        NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:14.0]};
        [str drawAtPoint:CGPointMake(10, 10) withAttributes:attribute];
    }
}
...
- (void) endEditing:(NSNotification*) notification {
    dispatch_async(dispatch_get_main_queue(), ^{ //!OCLINT
