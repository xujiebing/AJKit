...
#define kNavigationBarTitleColor kNavTintColor
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{ //!OCLINT
        [self swizzleMethod:@selector(setLeftBarButtonItem:) withMethod:@selector(bwtext_setLeftBarButtonItem:)];
    });
}

- (void)bwtext_setLeftBarButtonItem:(UIBarButtonItem *)leftBarButtonItem{
    if (leftBarButtonItem.customView) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 11 && [leftBarButtonItem.customView isKindOfClass:[UIButton class]]) {
            UIButton *customView = leftBarButtonItem.customView;
            UIButton *btn = customView;
            btn.frame = CGRectMake(-60, 0, customView.frame.size.width+60, 44);
            btn.contentEdgeInsets = UIEdgeInsetsMake(0, -45, 0, 0);
            UIBarButtonItem *backBtn = [[UIBarButtonItem alloc] initWithCustomView:btn];
            [self bwtext_setLeftBarButtonItem:nil];
            [self setLeftBarButtonItems:@[[self fixedSpaceWithWidth:-20], backBtn]];
        }else {
            [self bwtext_setLeftBarButtonItem:nil];
            [self setLeftBarButtonItems:@[[self fixedSpaceWithWidth:0], leftBarButtonItem]];
        }
    }else {
        [self setLeftBarButtonItems:nil];
        [self bwtext_setLeftBarButtonItem:leftBarButtonItem];
    }
}
...
+ (void)setLeftButtonOn:(UIViewController *)vc
    ...
    UIImage *image = BWTSkinImageNamed(@"msx_icon_back");
...
//    if (IOS11) {
//        btn.frame = CGRectMake(-60, 0, image.size.width+60, 44);
...
//    btn.showsTouchWhenHighlighted = YES;
    [btn setTitle:title forState:UIControlStateNormal];
    [btn setTitleColor:kMainUIColor forState:UIControlStateNormal];
    btn.titleLabel.font = [UIFont systemFontOfSize:13.f];
    btn.layer.borderColor = [kMainUIColor CGColor];
...
- (UIActivityIndicatorView *)activityIndicator {
    ...
    if(!activityIndicator) {
        ...
        activityIndicator.color = kNavigationBarTitleColor;
...
- (UILabel *)titleLabel {
    ...
    if (!titleLabel) {
        ...
        titleLabel.textColor = kNavigationBarTitleColor;
