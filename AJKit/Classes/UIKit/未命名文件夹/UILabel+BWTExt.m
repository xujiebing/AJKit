...
- (id<BWTAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<BWTAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)bwt_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self bwt_getRangesWithStrings:strings];
...
- (void)bwt_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <BWTAttributeTapActionDelegate> )delegate
{
    [self bwt_getRangesWithStrings:strings];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self bwt_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(bwt_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate bwt_attributeTapReturnString:string range:range index:index];
        }
        if (weakSelf.isTapEffect) {
            [weakSelf bwt_saveEffectDicWithRange:range];
            [weakSelf bwt_tapEffectWithStatus:YES];
        }
    }];

    if (self.isTapEffect) {
        [self performSelectorOnMainThread:@selector(bwt_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
    }
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self bwt_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)bwt_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self bwt_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self bwt_getLineBounds:line point:linePoint];
...
- (CGAffineTransform)bwt_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)bwt_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)bwt_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)bwt_saveEffectDicWithRange:(NSRange)range
{
...
- (void)bwt_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf bwt_getStringWithRange:range]];
...
- (NSString *)bwt_getStringWithRange:(NSRange)range
{
...
+ (NSMutableAttributedString *)wholeString:(NSString *)wholeString
                              targetString:(NSString *)targetString
                                     color:(UIColor *)color
                                      font:(UIFont *)font
{
    NSRange targetRange = [wholeString rangeOfString:targetString];
    if (targetRange.length == 0) {
        NSMutableAttributedString *arrString = [[NSMutableAttributedString alloc]initWithString:wholeString];
        return arrString;
    }
    NSMutableAttributedString *arrString = [[NSMutableAttributedString alloc]initWithString:wholeString];
    // 设置前面几个字串的格式:字号字体、颜色
 ...
 *    @param     font;                字号
 **/
+ (NSMutableAttributedString *)wholeAttributedString:(NSMutableAttributedString *)wholeString
                              addTargetString:(NSString *)targetString
                                     color:(UIColor *)color
                                      font:(UIFont *)font
{
    NSRange targetRange = [[wholeString string] rangeOfString:targetString];
    if (targetRange.length == 0) {
        return wholeString;
    }
    
    // 设置前面几个字串的格式:字号字体、颜色
    [wholeString addAttributes:@{NSFontAttributeName:font,
                               NSForegroundColorAttributeName:color
                               }
                       range:targetRange];
    return wholeString;
