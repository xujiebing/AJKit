...
#import "UIApplication+BWTExt.h"
#import "NSArray+BWTExt.h"
#import "UIDevice+BWTExt.h"

#include <sys/sysctl.h>
#include <mach/mach.h>


NSString * BWTDocumentsPath()
{
    return [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,
                                                NSUserDomainMask, YES) firstObject];
}

NSString *BWTLibraryPath() {
    return [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory,
                                                NSUserDomainMask, YES) firstObject];
}

NSString *BWTCachesPath() {
    return [NSSearchPathForDirectoriesInDomains(NSCachesDirectory,
                                                NSUserDomainMask, YES) firstObject];
}

@implementation UIApplication (BWTExt)
...
- (BOOL)isPirated {
    ...
    if (![self _BWT_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _BWT_fileExistInMainBundle:@"CodeResources"]) {
        return YES;
    }
    
    if (![self _BWT_fileExistInMainBundle:@"ResourceRules.plist"]) {
        return YES;
    }
...
- (BOOL)_BWT_fileExistInMainBundle:(NSString *)name {
...
- (BOOL)isBeingDebugged {
    ...
    return (info.kp_proc.p_flag & P_TRACED) ? YES : NO; //!OCLINT
}
...
- (float)cpuUsage {
    ...
    for (j = 0; j < thread_count; j++) {
        ...
        if (!(basic_info_th->flags & TH_FLAGS_IDLE)) { //!OCLINT
