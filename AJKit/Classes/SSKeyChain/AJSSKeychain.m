...
NSString *const kAJSSKeychainErrorDomain = @"com.samsoffes.AJSSKeychain";

NSString *const kAJSSKeychainAccountKey = @"acct";
NSString *const kAJSSKeychainCreatedAtKey = @"cdat";
NSString *const kAJSSKeychainClassKey = @"labl";
NSString *const kAJSSKeychainDescriptionKey = @"desc";
NSString *const kAJSSKeychainLabelKey = @"labl";
NSString *const kAJSSKeychainLastModifiedKey = @"mdat";
NSString *const kAJSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef AJSSKeychainAccessibilityType = NULL;
#endif

@interface AJSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation AJSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = AJSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kAJSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
        AJSSKeychainAccessibilityType = kSecAttrAccessibleAlways;
		if (AJSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return AJSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (AJSSKeychainAccessibilityType) {
		CFRelease(AJSSKeychainAccessibilityType);
	}
	AJSSKeychainAccessibilityType = accessibilityType;
}
#endif
